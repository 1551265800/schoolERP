<template>
  <div class="container">
    <Breadcrumb :items="['查看信息', '采购统计']" class="mytable"/>
    <a-card class="general-card" style="padding-top: 20px;">
            ...
            <a-row :gutter="16">
              <a-col :span="16">
                <a-form-item field="time">
                  <a-button type="primary" @click="cTime" style="position: relative; right: 15px;">
                    选择时间
                  </a-button>
      ...
      <a-table row-key="id" :loading="loading" :columns="(cloneColumns as TableColumnData[])" :data="renderData"
        :bordered="{ cell: true }" :size="size" :pagination="false" >
...
const getList = async () => {
  ...
  form.value.time = time.nowDateStr;
...
const fetchData = async (params?: any) => {
  setLoading(true);
  try {
    const { data } = await caiGouList(params);
    if (data.length == 0) {
...
const modifyAndPrepare = async (info: any) => {
  try {
    setLoading(true);
    const res: any = await editorialOrganization({
      id: info.id,
