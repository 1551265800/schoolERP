      ...
      <a-row>
        <a-col :flex="1">
          ...
          >
            <a-row :gutter="16">
              <a-col :span="16">
                <a-form-item field="organName" label="组织名称">
                  <a-input
                  :style="{ width: '320px' }"
              ...
              <a-col :span="16">
                <a-form-item field="legalPerson" label="代表法人">
                  <a-input
                  :style="{ width: '320px' }"
      ...
      >
        <a-form :model="emitForm" auto-label-width>
          ...
          <a-form-item
            ...
            label="创立时间："
            :rules="[{ required: true, message: '请选择出创立时间' }]"
          >
            <a-date-picker
              placeholder="请选择创立时间"
          ...
          <a-form-item
            field="legalPerson"
            label="代表法人："
            :rules="[{ required: true, message: '请输入代表法人' }]"
          >
            <a-input
          ...
          <a-form-item
            field="memberNum"
            label="成员规模"
            :rules="[{ required: true, message: '请输入成员规模' }]"
          >
            <a-input-number 
...
const mychange = () => {
  nextTick(() => {
    emitForm.organName = emitForm.organName.replace(/\s+/g, "");
  });
};
...
const handleBeforeOk = async (done: any) => {
  ...
  if (!emitForm.organName) {
    Message.error({
      content: "请输入组织名称",
    });
    return false;
  }
  if (!emitForm.establishedTime) {
    Message.error({
      content: "请输入创立时间",
    });
    return false;
...
const systemDetail = async () => {
  ...
  try {
    ...
    emitForm.organName = detailData.data.organName;
