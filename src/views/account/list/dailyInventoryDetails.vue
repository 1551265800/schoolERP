<template>
  <div class="container">
    <Breadcrumb :items="['menu.system', '每日库存详情']" class="mytable" />
            ...
            <a-row :gutter="16">
              <a-col :span="16">
                <a-form-item field="foodName" label="食材名称">
                  <a-select v-model="form.foodName" placeholder="请选择食材名称" :options="data" style="width: 250px"  allow-search></a-select>
                </a-form-item>
              </a-col>
            </a-row>
            <a-row :gutter="16">
              <a-col :span="16">
                <a-form-item field="time" label="生成记录时间">
                  <a-date-picker v-model="form.time" format="YYYY-MM-DD" style="width: 250px"/>
                  <!-- @ok="onOk" /> -->
                </a-form-item>
              </a-col>
            </a-row>
...
import { dailyInventoryDetails } from "@/api/outboundRecord";
...
const fetchData = async (params?: any) => {
  setLoading(true);
  try {
    const { data } = await dailyInventoryDetails(params);
    if (data.length == 0) {
