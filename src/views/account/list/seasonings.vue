<template>
  <div class="container">
    <Breadcrumb :items="['menu.system', '调料盘点']" class="mytable" />
    <a-card class="general-card" style="padding-top: 20px">
        ...
        <div class="item" v-for="(v, index ) in renderData2" :key="index">
          ...
          <div class="item-right">
            <div class="line">
              ...
              <text class="chef">库存：{{ v.quantity }}({{ v.unit }})&nbsp;&nbsp;</text>
              <br>
              <text class="chef">规格：{{ v.specificationWeight }}(斤)&nbsp;&nbsp;</text>
              <br>
            </div>
          </div>
          <div class="LYbtn2" @click="det(v.id, v.foodName, v.unit)">
      ...
      <a-modal v-model:visible="isShow" :title="or ? '新增' : '编辑'" @cancel="myClose" @before-ok="handleBeforeOk"
        ...
        <a-form :model="emitForm" auto-label-width>
          ...
          <a-form-item field="unit" label="单位" :rules="[{ required: true, message: '请选择单位' }]">
            <a-select v-model="emitForm.unit" placeholder="请选择单位" :options="data2"></a-select>
          </a-form-item>
          ...
          <a-form-item field="specificationWeight" label="规格重量" :rules="[{ required: true, message: '请输入重量' }]">
            <a-input-number v-model="emitForm.specificationWeight" @input="mychange" :max-length="10" placeholder="请输入规格重量" />
          </a-form-item>
        </a-form>
      </a-modal>
    </a-card>
...
const fetchData = async (params?: any) => {
  setLoading(true);
  try {
    // const { data } = await seasoningCategory(params);
    const data2 = await seasoningCategory(params);
    // if (data.length == 0) {
...
const handleBeforeOk = async (done: any) => {
  ...
  if (!emitForm.specificationWeight) {
    Message.error({
      content: "请输入规格重量",
    });
    return false;
  ...
  try {
    setLoading(true);
    const res: any = await addSeasoningCategory(emitForm);
    if (res.msg) {
      Message.success({
        content: res.msg,
      });
...
const systemDetail = async () => {
  ...
  try {
    ...
    emitForm.specificationWeight = detailData.data.specificationWeight;
  } catch (err) {
    // you can report use errorHandler or other
  } finally {
    setLoading(false);
...
const det = (pri: any, name: any, unit: any) => {
  // console.log(pri);
  // router.push({ name: 'seasondetil', params: { pri } })
  router.push({ path: 'seasondetil', query: { foodId: pri, foodName: name, unit: unit } })
}
</script>
